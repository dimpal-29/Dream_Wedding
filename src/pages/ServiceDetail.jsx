...
function ServiceDetail() {
  ...
  const category = searchParams.get('category') || 'decoration'
  const data = SERVICE_DATA[category] || SERVICE_DATA['decoration']

  const handleBookNow = (serviceName, defaultEventType) => {
    const isLoggedIn = localStorage.getItem('dreamWedding_loggedIn') === 'true'
    
    if (!isLoggedIn) {
      window.showToast('Please login to book a service.', 'warning')
      setTimeout(() => navigate('/login?redirect=' + encodeURIComponent('/book-now?service=' + encodeURIComponent(serviceName) + '&category=' + encodeURIComponent(category) + '&eventType=' + encodeURIComponent(defaultEventType))), 1200)
      return
    }

    navigate('/book-now?service=' + encodeURIComponent(serviceName) + '&category=' + encodeURIComponent(category) + '&eventType=' + encodeURIComponent(defaultEventType))
  }
    ...
    <>
      ...
      <section className="page-hero">
        <div className="container">
          <h1 id="categoryTitle">{data.title}</h1>
          <p id="categoryDesc">{data.desc}</p>
        </div>
      </section>
      ...
      <section className="service-detail-section">
        <div className="container">
          <div id="serviceItemsGrid" className="service-items-grid">
            {data.items.map((item, index) => (
              <article key={index} className="sub-service-card">
                <div className="sub-service-img">
                  <img src={item.img} alt={item.name} />
                </div>
                <div className="sub-service-content">
                  <h3 className="sub-service-title">{item.name}</h3>
                  <p className="sub-service-desc">{item.desc}</p>
                  <div className="sub-service-actions" style={{ marginTop: 'auto', paddingTop: '10px' }}>
                    <button 
                      className="btn btn-primary" 
                      onClick={() => handleBookNow(item.name, item.defaultEventType || '')}
                    >
                      Book Now
                    </button>
                  </div>
